...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(height =  "50%",
            ...
            sliderInput("ano_loa", "ano do empenho/autorização do investimento:",
            ...
            sliderInput("ano_pgt", "ano do pagamento do investimento:",
        ...
        mainPanel(
           plotOutput("ano_lei", height =  180),
           plotOutput("ano_pgt", height =  180)
        )
    )
)
...
server <- function(input, output) {
    ...
    output$ano_lei <- renderPlot({
      prisma_lei<- prisma%>%
        ...
        ggplot(prisma_lei) +
          ...
          ggtitle("ano do empenho/autorização do investimento (R$ bi)")+
          theme(plot.title=element_text(family='', face='bold', colour='#cc6600', size=11))+
          theme(axis.title.x = element_blank())+
          theme(axis.title.y = element_blank())+
          theme(axis.text.x = element_text(size = 7)) +
          geom_text(aes(x=ano_lei, y=pago, label = round(pago,2)), size = 2.5,  vjust = -0.5)+ 
          theme(
            # Remove panel border
    ...
    output$ano_pgt <- renderPlot({
      ...
      ano<-plot_ly(prisma_pago)%>%
        ...
        theme_bw()+
        scale_x_continuous(limits=c(2007,2020), breaks  = c(2008, 2009,2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2017, 2018, 2019))+
        ggtitle("ano do pagamento do investimento (R$ bi)")+
        theme(plot.title=element_text(family='', face='bold', colour='#ff0000', size=11))+
        theme(axis.title.x = element_blank())+
        theme(axis.title.y = element_blank())+
        theme(axis.text.x = element_text(size = 7)) + 
        geom_text(aes(x=ano_despesa, y=pago, label = round(pago,2)), size = 2.5 ,  vjust = -0.5)+ 
        theme(
          # Remove panel border
          panel.border = element_blank(),  
          # Remove panel grid lines
